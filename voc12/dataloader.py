...
class VOC12ImageDataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        return {'name_ny':name ,'name': name_str, 'img': img}


# base_img_ref_path = './voc12/list/half/'
base_img_ref_path = './voc12/list/full/'
...
class VOC12ClassificationDataset(VOC12ImageDataset):
    ...
    def __getitem__(self, idx):
        ...
        # self.img_name_list == out_tar['name_ny']

        ref_list = [i for i, value in enumerate(out['label']) if value == 1]
        idx_ref = random.choice(ref_list)
        idx_ref_class = idx_ref + 1

        ref_name = get_ref_img(idx_ref_class)
        ref_name_ny = int(ref_name)

        ref_idx = np.where(self.img_name_list == ref_name_ny)
        ref_idx = ref_idx[0].item()

        out_ref = super().__getitem__(ref_idx)
        out_ref['label'] = torch.from_numpy(self.label_list[ref_idx])

        out['name_ref'] = out_ref['name']
        out['img_ref'] = out_ref['img']
        out['label_ref'] = out_ref['label']

        out['common_cls'] = idx_ref
...
class VOC12ClassificationDatasetMSF(VOC12ClassificationDataset):
    ...
    def __getitem__(self, idx):
        ...
        img_ref = imageio.imread(get_img_path(ref_name, self.voc12_root))
        img_ref = self.img_normal(img_ref)
        img_ref = imutils.HWC_to_CHW(img_ref)
        img_ref = np.stack([img_ref, np.flip(img_ref, -1)], axis=0)

        out = {"name": name_str, "img": ms_img_list, "size": (img.shape[0], img.shape[1]),
               "label": torch.from_numpy(self.label_list[idx]) ,"img_ref":img_ref , 'common_cls':idx_ref}
        return out
