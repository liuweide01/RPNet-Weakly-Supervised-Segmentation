...
class PathIndex:
    ...
    def get_search_paths_dst(self, max_radius=5):
        ...
        coord_indices_by_length = [[] for _ in range(max_radius * 4)]   # a list contain the number of length of [] .

        search_dirs = []

        for x in range(1, max_radius):
            search_dirs.append((0, x))  # For first dimension

        for y in range(1, max_radius):
            for x in range(-max_radius + 1, max_radius):
                if x * x + y * y < max_radius ** 2:
                    search_dirs.append((y, x))  # list all the search circles
        ...
        for dir in search_dirs:
            ...
            coord_indices_by_length[path_length].append(path_coords)    # sort by distance
    ...
    def get_path_indices(self, size):
        ...
        cropped_width = size[1] - 2 * self.radius_floor # Define the image size

        path_indices = []

        for paths in self.search_paths: #(b,c,x,y)

            path_indices_list = []
            for p in paths:  # (c,x,y)
...
if __name__ == "__main__":
    path_index = PathIndex(radius=5, default_size=(512 // 4, 512 // 4))
    n_path_lengths = len(path_index.path_indices)
